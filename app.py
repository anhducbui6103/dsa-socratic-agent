...
st.markdown(
    ...
    /* Không để body/page tạo outer scroll. Scroll nằm trong từng st.container riêng. */
    ...
    /* Các container có height trong Streamlit chính là các scroll box riêng */
...
left_col, middle_col, right_col = st.columns([0.18, 0.55, 0.27], gap="small")

with left_col:
    st.markdown('<div class="left-root"></div>', unsafe_allow_html=True)

    # Scroll box riêng của sidebar trái
...
with middle_col:
    st.markdown('<div class="middle-root"></div>', unsafe_allow_html=True)
    session = current_session()

    # Scroll box riêng của phần chat history
    ...
    # Input nằm riêng bên dưới, không nằm trong scroll box
...
with right_col:
    st.markdown('<div class="right-root"></div>', unsafe_allow_html=True)

    # Scroll box riêng của sidebar phải
    with st.container(height=830, border=False):
        session = current_session()
        graph = session.graph
